...
public class GrassyPlains extends Game
{
    ...
    private void spawnProps()
    {
        for (int i = 0; i < 35; i++) {
            ...
            addObject(new Prop(flower, x, y), 0, 0);
        }
        ...
        for (int i = 0; i < 12; i++) {
            ...
            addObject(new Prop(tree, x, y), 0, 0);
        }
        ...
        for (int i = 0; i < 8; i++) {
            ...
            addObject(new Prop(col, x, y, true), 0, 0);
        }

        for (int i = 0; i < 6; i++) {
            double x = Greenfoot.getRandomNumber((int)WORLD_WIDTH);
            double y = Greenfoot.getRandomNumber((int)WORLD_HEIGHT);
            addObject(new Prop("забор_вертикаль.png", x, y, true), 0, 0);
        }

        for (int i = 0; i < 5; i++) {
            double x = Greenfoot.getRandomNumber((int)WORLD_WIDTH);
            double y = Greenfoot.getRandomNumber((int)WORLD_HEIGHT);
            addObject(new Prop("забор_горизонт1.png", x, y, true), 0, 0);
        }
    }
}